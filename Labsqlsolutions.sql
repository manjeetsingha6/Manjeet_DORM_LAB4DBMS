...
-- Solution-3)--
select  c.cus_gender, count(*), sum(o.ord_amount) from customer as c inner join `order` as o on o.cus_id = c.cus_id where ord_amount >= 3000 group by c.cus_gender;

-- Solution-4)--
select o.ord_id, p.pro_name, o.ord_amount, o.ord_date, o.cus_id from product as p, supplier_pricing as s, `order` as o  where s.pricing_id = o.pricing_id and s.pro_id = p.pro_id and o.cus_id = 2;
...
-- Solution-6)--
select c.cat_id, c.cat_name, p.pro_name, min(s.supp_price) from catogory as c, product as p, supplier_pricing as s where c.cat_id = p.cat_id and s.pro_id = p.pro_id group by c.cat_id order by c.cat_id asc;

-- Solution-7)--
select p.pro_id, p.pro_name, o.ord_date from product as p, supplier_pricing as s, `order` as o where s.pro_id = p.pro_id and o.pricing_id = s.pricing_id and o.ord_date > '2021-10-05';
...
BEGIN
select su.supp_id , su.supp_name, avg(r.rat_stars),
case
